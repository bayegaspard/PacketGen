...
def plot_incremental_confusion_matrices(model, diffusion_model, X_test, y_test, label_encoder, steps, selected_classes=None):
    ...
    if selected_classes:
        X_test, y_test = filter_data_by_classes(X_test, y_test, label_encoder, selected_classes)
        print(f"Filtered to classes: {selected_classes}")
        # Update labels to only show selected classes
        labels_to_display = [cls for cls in label_encoder.classes_ if cls in selected_classes]
    else:
    ...
    for step in steps:
        ...
        # Plot confusion matrix
        cm = confusion_matrix(y_test.cpu(), y_noisy_pred, labels=[label_encoder.transform([cls])[0] for cls in labels_to_display])
        plt.figure(figsize=(10, 8))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=labels_to_display, yticklabels=labels_to_display)
        plt.title(f'Confusion Matrix at Noise Step {step} for Selected Classes')
        plt.ylabel('Actual')
        plt.xlabel('Predicted')
        plt.xticks(rotation=45)
