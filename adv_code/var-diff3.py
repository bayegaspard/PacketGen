# diffusion_model_dnn_analysis_corrected.py
...
def train_classifier(model, train_loader, criterion, optimizer, num_epochs=2, validation_loader=None):
    model.train()
    train_losses = []
    val_f1_scores = []
    val_accuracy_scores = []
...
def evaluate_classifier(model, data_loader, label_encoder, description=''):
    model.eval()
    y_pred = []
    y_true = []
    y_scores = []
...
def sensitivity_analysis_classifier(model, data_loader, label_encoder, description=''):
    """
    ...
    # Plot sensitivity per feature
    plt.figure(figsize=(20, 10))
    sns.barplot(x=list(range(len(sensitivity_per_feature))), y=sensitivity_per_feature, color='blue')
    plt.title(f"Sensitivity of Classifier to Input Features ({description})")
...
def shap_analysis_classifier(model, data_loader, label_encoder, description=''):
    """
    ...
    # Plot summary
    plt.figure(figsize=(20, 10))
    shap.summary_plot(shap_values, features=X_explain, feature_names=features.columns, show=False, plot_type='dot')
    plt.title(f'SHAP Summary Plot ({description})')
    plt.tight_layout()
    ...
    plt.figure(figsize=(20, 10))
    shap.summary_plot(shap_values, features=X_explain, feature_names=features.columns, plot_type='bar', show=False)
    plt.title(f'SHAP Feature Importance Bar Plot ({description})')
    plt.tight_layout()
...
def analyze_model_parameters(model, description=''):
    ...
    # Plot histograms of weights and biases
    ...
    for i, (w, name) in enumerate(zip(weights, layer_names)):
        ...
        if i < len(biases):
            ...
            plt.title(f'Bias Distribution: {name.replace("weight", "bias")}')
    ...
    # Plot parameter norms per layer
    weight_norms = []
    bias_norms = []
    layers = []
    for name, param in model.named_parameters():
    ...
    plt.xlabel('Layer')
    plt.ylabel('L2 Norm')
    plt.title(f'Parameter Norms per Layer ({description})')
    plt.xticks(index + bar_width / 2, layers, rotation=90)
    plt.legend()
    plt.tight_layout()
    ...
    # Plot distribution of weight magnitudes across layers
...
def train_diffusion_model(model, diffusion, train_loader, optimizer, num_epochs=2):
    model.train()
    for epoch in range(num_epochs):
        epoch_loss = 0.0
...
def evaluate_on_perturbed_data_at_steps(model, diffusion_model, diffusion, X_test_combined, y_test_combined, label_encoder, incremental_steps, unknown_label, batch_size=256, model_description=''):
    ...
    for t in incremental_steps:
        ...
        plt.title(f'Confusion Matrix (t={t}) - {model_description}')
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.xticks(rotation=45)
        plt.yticks(rotation=45)
...
def plot_metrics(metrics_df):
    """
    Plots performance metrics against perturbation steps.
    ...
    for metric in metrics_list:
        ...
        plt.title(f'{metric} vs Perturbation Step')
        plt.xlabel('Perturbation Step')
        plt.ylabel(metric)
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'results/{metric}_vs_Perturbation_Step.png', dpi=300)
        plt.close()
        print(f"{metric} vs Perturbation Step plot saved as 'results/{metric}_vs_Perturbation_Step.png'")
    
    # Additionally, plot all metrics in a single plot for each model
    for model in models:
        subset = metrics_melted[metrics_melted['Model'] == model]
        plt.figure(figsize=(10, 6))
        sns.lineplot(data=subset, x='Perturbation_Step', y='Value', hue='Metric', marker='o')
        plt.title(f'Performance Metrics vs Perturbation Step - {model}')
        plt.xlabel('Perturbation Step')
        plt.ylabel('Metric Value')
        plt.legend(title='Metric')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'results/Metrics_vs_Perturbation_Step_{model}.png', dpi=300)
        plt.close()
...
def main():
    ...
    train_diffusion_model(diffusion_model, diffusion, train_loader, diffusion_optimizer, num_epochs=2)
    ...
    for config in model_configs:
        ...
        train_losses, val_f1_scores, val_accuracy_scores, val_roc_auc_scores = train_classifier(
            model, train_loader, criterion, optimizer, num_epochs=2, validation_loader=test_loader_known
        )
        
        # Evaluate on clean test data (known classes only)
        evaluate_classifier(model, test_loader_known, label_encoder, description=config['description'] + '_clean')
        
        # Sensitivity Analysis (Gradients)
        sensitivity = sensitivity_analysis_classifier(model, test_loader_known, label_encoder, description=config['description'])
        
        # SHAP Analysis
        explainer, shap_values = shap_analysis_classifier(model, test_loader_known, label_encoder, description=config['description'])
    ...
    for result in experiment_results:
        ...
        for t in incremental_steps:
            ...
            sns.heatmap(cm, annot=True, fmt='d', cmap='Reds',
                        xticklabels=extended_label_encoder.classes_,
                        yticklabels=extended_label_encoder.classes_)
            plt.title(f'Confusion Matrix (t={t}) - {description}')
            plt.xlabel('Predicted Label')
            plt.ylabel('True Label')
            plt.xticks(rotation=45)
            plt.yticks(rotation=45)
